...
class ModelLoader {
  public:
    ...
    static void processNode(aiNode *node, const aiScene *scene, std::string &directory, std::vector<WorldMesh> &meshes,
                            std::map<string, BoneInfo> &boneInfo, int &boneCountDst) {
        // process each mesh located at the current node
        for (unsigned int i = 0; i < node->mNumMeshes; i++) {
            // the node object only contains indices to index the actual objects in the scene.
            // the scene contains all the data, node is just to keep stuff organized (like relations between nodes).
            aiMesh *mesh = scene->mMeshes[node->mMeshes[i]];
            meshes.push_back(processMesh(mesh, scene, directory, boneInfo, boneCountDst));
        }
        // after we've processed all of the meshes (if any) we then recursively process each of the children nodes
        for (unsigned int i = 0; i < node->mNumChildren; i++) {
            processNode(node->mChildren[i], scene, directory, meshes, boneInfo, boneCountDst);
        }
    }
